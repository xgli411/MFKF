...
def detect(opt):
    ...
    # t0 = time.time()
    # total_time = 0  # 记录总的推理时间
    ap = actionPredictor()
    detected_images = []
    is_video=True
    for path, img, im0s, vid_cap in dataset:
        # start_time = time.time()  # 记录当前图片的开始时间
        ...
        # total_time += time.time() - start_time  # 计算当前图片的推理时间并累加到总的推理时间中
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(det[:,:6]):
                    ...
                    if save_txt:  # Write to file
                        ...
                        centerpoint.append((float(xywh[0]), float(xywh[1])))
                        left_top_coord.append((int(xyxy[0]), int(xyxy[1])))
                        right_bottom_coord.append((int(xyxy[2]), int(xyxy[3])))
                        ...
                        with open(txt_path + '.txt', 'a') as f:
                            ...
                            for kpt in kpts:
                                f.write(f'{kpt:.2f} ')  # Add keypoint info
                            f.write('\n')
                    joints_humans = []
                    boundingboxes = []
                    scores = []
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        kpts = det[det_index, 6:]
                        kpts = kpts.detach().cpu().numpy()
                        boundingbox = [int(xy) for xy in xyxy]
                        joints_humans.append(kpts)
                        scores.append(conf)
                        boundingboxes.append(boundingbox)
                    statuses = ap.analyze_joints(joints_humans, boundingboxes, frame,is_video=is_video,left_top_coord=left_top_coord,right_bottom_coord=right_bottom_coord)
                    statuses = str(statuses)
                    # check_status(statuses, centerpoint)


                    # plot_one_box(boundingbox, im0, statuses=statuses,label=label, color=colors(c, True),
                    #             line_thickness=opt.line_thickness, kpt_label=kpt_label, kpts=kpts, steps=3,
                    #             orig_shape=im0.shape[:2])
                    plot_one_box(boundingbox, im0, label=label, color=colors(c, True),
                                line_thickness=opt.line_thickness, kpt_label=kpt_label, kpts=kpts, steps=3,
                                orig_shape=im0.shape[:2])
                    # detected_images.append((np.array(processed_image)))

                    if opt.save_crop:
                        save_one_box(statuses,boundingbox, im0s, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg',
                                     BGR=True)
        ...
        detected_images.append((np.array(im0)))
    ...
    # total_fps = len(dataset) / total_time
    # print(
    #     f'Inference time: {inference_time:.2f}s, FPS: {fps:.2f}, Total time: {total_time:.2f}s, Total FPS: {total_fps:.2f}')
    # detected_images = np.array(im0)
    return joints_humans,scores
